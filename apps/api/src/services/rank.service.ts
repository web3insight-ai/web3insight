...
import { ECO_ALL, type EcoNameFilter } from '@/data/dto/data.dto';
import {
  type EcoRankDto,
  EcoRankListDto,
  type TotalDto,
...
/**
 * Pure-class port of data/services/rank.services.ts. Console @Command decorators
 * stripped; the sync:eco:rank command body is exposed as `syncAll()`.
 */
export class RankService {
  ...
  async ecoRankTotal(ecoName: EcoNameFilter, cache: boolean = true) {
    const dbData = await this.cacheService.get(CacheKey.EcoRank, ecoName);
    ...
    for (const ecoName of ecoTypes) {
      const actorsTotalAllScopeResult = await this.cacheService.get(
        CacheKey.ActorTotal,
        ecoName,
      );

      const actorsTotalCoreScopeResult = await this.cacheService.get(
        CacheKey.ActorCoreTotal,
        ecoName,
      );

      const actorsNewTotal = await this.cacheService.get(
        CacheKey.ActorTotalNew,
        ecoName,
      );

      const totalRepos = await this.cacheService.get(
        CacheKey.RepoTotal,
        ecoName,
      );
    ...
    await this.cacheService.update(
      CacheKey.EcoRank,
      cacheData as unknown as Record<string, unknown>,
...
ORDER BY ecosystem;`;
  ...
  async getEcoRepoRank(ecoNames: string[]) {
    ...
    for (const row of results.rows as Array<Record<string, unknown>>) {
      const ecoName = row.eco_name as string;
      if (!groupedResults[ecoName]) {
        groupedResults[ecoName] = [];
      }

      groupedResults[ecoName].push({
        eco_name: ecoName,
...
`;
  ...
  /** Original sync:eco:rank command body — exposed for Cron/Inngest in Phase F. */
  async syncAll() {
  ...
  /** Original sync:eco:rank:new command body. */
  async syncEcoRepoRank() {
    const ecoTypes = await this.ecoService.getActiveEcoNames();
