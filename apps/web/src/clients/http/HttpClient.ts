...
class HttpClient {
  ...
  private async request(url: string, config: RequestConfig = {}) {
    ...
    const signal = (config as RequestConfigWithTimeout).signal || controller.signal;
  ...
  get(url: string, config?: Omit<RequestConfigWithTimeout, 'method' | 'body'>) {
    return this.request(url, { ...config, method: 'GET' });
  }

  post(url: string, data?: unknown, config?: Omit<RequestConfigWithTimeout, 'method'>) {
    return this.request(url, { ...config, method: 'POST', body: data });
  }

  put(url: string, data?: unknown, config?: Omit<RequestConfigWithTimeout, 'method'>) {
    return this.request(url, { ...config, method: 'PUT', body: data });
  }

  delete(url: string, config?: Omit<RequestConfigWithTimeout, 'method' | 'body'>) {
    return this.request(url, { ...config, method: 'DELETE' });
  }

  patch(url: string, data?: unknown, config?: Omit<RequestConfigWithTimeout, 'method'>) {
