...
const Thread: FC<ThreadProps> = ({ onMinimize, onClose }) => {
  ...
  return (
    <ThreadPrimitive.Root className="flex flex-col h-full">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-rule bg-bg-sunken">
        <div className="flex items-center gap-2">
          <Sparkles size={16} className="text-accent" />
          <span className="font-display font-semibold text-sm tracking-[-0.01em]">
            Web3Insight AI
          </span>
        </div>
        <div className="flex items-center gap-1">
          {hasMessages && (
            <button
              ...
              className="p-1.5 text-fg-muted hover:text-fg hover:bg-bg-raised rounded-[2px] transition-colors"
              title="New chat"
            >
          ...
          {isRunning && (
            <button
              type="button"
              onClick={handleStop}
              className="flex items-center gap-1 px-2 py-1 text-xs text-fg-muted hover:text-danger hover:bg-accent-subtle rounded-[2px] transition-colors"
            >
              <StopCircle size={12} />
              Stop
            </button>
        ...
        <ThreadPrimitive.Empty>
          <div className="p-4">
            ...
            <div className="space-y-2">
              <p className="text-xs text-fg-muted mb-2">Try asking:</p>
              {EXAMPLE_QUERIES.map((example) => (
                <button
                  key={example}
                  type="button"
                  onClick={() => handleSuggestionClick(example)}
                  className="block w-full text-left px-3 py-2 text-xs text-fg bg-bg-sunken hover:bg-bg-raised border border-rule rounded-[2px] transition-colors"
                >
                  {example}
                </button>
              ))}
