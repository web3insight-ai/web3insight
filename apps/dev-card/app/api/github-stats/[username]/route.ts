...
export async function GET(
  ...
  try {
    ...
    // Fetch GitHub stats from yu-readme service (same as DevInsight dashboard)
    const [statsResponse, langsResponse] = await Promise.all([
      fetch(
        `https://yu-readme.vercel.app/api?username=${username}&count_private=true`,
      ),
      fetch(
        `https://yu-readme.vercel.app/api/top-langs/?username=${username}&hide=markdown,html,css,Svelte,smarty`,
      ),
    ])
    ...
    const parseGitHubStats = (svg: string) => {
      ...
      const starsMatch = desc.match(
        /Total Stars Earned:\s*(\d+(?:[.,]\d+)?[kmb]?)/i,
      )
      const commitsMatch = desc.match(
        /Total Commits.*?:\s*(\d+(?:[.,]\d+)?[kmb]?)/i,
      )
      const prsMatch = desc.match(
        /Total PRs:\s*(\d+(?:[.,]\d+)?[kmb]?)/i,
      )
      const issuesMatch = desc.match(
        /Total Issues:\s*(\d+(?:[.,]\d+)?[kmb]?)/i,
      )
      const contributedMatch = desc.match(
        /Contributed to.*?:\s*(\d+(?:[.,]\d+)?[kmb]?)/i,
      )
    ...
    const parseTopLanguages = (svg: string) => {
      const langNameRegex =
        /<text[^>]*data-testid="lang-name"[^>]*>([^<]+)<\/text>/g
