...
async function fetchUserData(userId: string) {
  try {
    const response = await fetch(
      `${env.DATA_API_URL}/v2/auth/user/info/mantle/${userId}`,
      { next: { revalidate: 60 } }
    )
...
export async function generateMetadata({
  ...
  const ogImageUrl = `${baseUrl}/api/og/mantle/${user_id}`
  const pageUrl = `${baseUrl}/mantle/${user_id}`
