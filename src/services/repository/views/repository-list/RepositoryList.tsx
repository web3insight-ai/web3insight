...
import { Search, Database, Star, GitFork, Filter, SortAsc, SortDesc } from "lucide-react";
        ...
        <Card className="bg-white dark:bg-surface-dark shadow-subtle border border-border dark:border-border-dark">
          <CardBody className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-500">Total Repositories</p>
        ...
        <Card className="bg-white dark:bg-surface-dark shadow-subtle border border-border dark:border-border-dark">
          <CardBody className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-500">Marked Repositories</p>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
        ...
        <div className="flex gap-2">
          <Dropdown>
            ...
            <DropdownMenu aria-label="Sort options">
              <DropdownItem key="fullName" onClick={() => handleSortChange("fullName")}>
              ...
              <DropdownItem key="customMark" onClick={() => handleSortChange("customMark")}>
                <div className="flex items-center justify-between w-full">
              ...
              {stats.hasStarData && (
                <DropdownItem key="star" onClick={() => handleSortChange("star")}>
                  <div className="flex items-center justify-between w-full">
                    <span>Stars</span>
              ...
              {stats.hasForkData && (
                <DropdownItem key="fork" onClick={() => handleSortChange("fork")}>
                  <div className="flex items-center justify-between w-full">
                    <span>Forks</span>
      ...
      <Card className="bg-white dark:bg-surface-dark shadow-subtle border border-border dark:border-border-dark overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-t border-border dark:border-border-dark bg-surface dark:bg-surface-dark">
                ...
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-500 uppercase tracking-wider">Custom Mark</th>
            ...
            <tbody className="divide-y divide-border dark:divide-border-dark">
              {loading ? (
                <tr>
                  <td colSpan={4 + (stats.hasStarData ? 1 : 0) + (stats.hasForkData ? 1 : 0)} className="px-6 py-16 text-center">
                    <div className="flex items-center justify-center">
                      <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary" />
                      <span className="ml-3 text-gray-500 dark:text-gray-400">Loading repositories...</span>
                    </div>
                  </td>
                </tr>
                      ...
                      {stats.hasStarData && (
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <span className="text-gray-700 dark:text-gray-300 font-mono text-sm">
                            {Number(repo.statistics?.star || 0).toLocaleString()}
                          </span>
                        </td>
                      )}
                      {stats.hasForkData && (
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <span className="text-gray-700 dark:text-gray-300 font-mono text-sm">
                            {Number(repo.statistics?.fork || 0).toLocaleString()}
                          </span>
                        </td>
                      )}
        ...
        <div className="px-6 py-4 border-t border-border dark:border-border-dark flex justify-between items-center">
          <div className="text-sm text-gray-500 dark:text-gray-400">
            ...
            {Math.min(pagination.pageNum * pagination.pageSize, pagination.total)} of {pagination.total} repositories
          </div>
