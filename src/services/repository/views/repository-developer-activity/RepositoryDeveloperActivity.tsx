...
function RepositoryDeveloperActivityView({ className, dataSource }: RepositoryDeveloperActivityViewWidgetProps) {
  ...
  const normalizedData = useMemo<ChartDatum[]>(() => {
    ...
    return dataSource
      .map(repo => ({
        ...
        developerCount: Number.isFinite(Number(repo.dev_7_day)) ? Number(repo.dev_7_day) : 0,
  ...
  const filteredData = useMemo<ChartDatum[]>(() => {
    ...
    return normalizedData.filter(repo => !SENSITIVE_REPOSITORIES.has(repo.repoName));
  }, [normalizedData, includeSensitive]);

  const chartData = useMemo<ChartDatum[]>(() => filteredData.slice(0, 10), [filteredData]);
        ...
        <div className="bg-white dark:bg-surface-dark border border-border dark:border-border-dark rounded-lg p-3 shadow-subtle max-w-xs">
          ...
          <div className="space-y-1 text-xs">
            <div className="flex items-center justify-between">
              <span className="text-gray-500 dark:text-gray-400">Active Developers (7d)</span>
              <span className="font-semibold text-primary">{formatNumber(data.developerCount)}</span>
            </div>
            <div className="flex items-center justify-between">
        ...
        <div className="flex items-start justify-between gap-4 w-full">
          ...
          <div className="flex items-center gap-3 whitespace-nowrap">
            <button
              ...
              aria-label={includeSensitive ? "Hide flagged repositories" : "Show flagged repositories"}
              title={includeSensitive ? "Hide flagged repositories" : "Show flagged repositories"}
            >
              {includeSensitive ? <Eye size={9} strokeWidth={1.8} /> : <EyeOff size={9} strokeWidth={1.8} />}
            </button>
      ...
      <CardBody className="px-6 pb-6 pt-0 space-y-6">
        {hasData ? (
            ...
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Bar dataKey="developerCount" fill="#0D9488" radius={[6, 6, 6, 6]} barSize={16} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="h-72 w-full flex items-center justify-center text-sm text-gray-500 dark:text-gray-400 text-center px-6">
            Flagged repositories are hidden. Toggle the button above to view them.
          </div>
        )}
