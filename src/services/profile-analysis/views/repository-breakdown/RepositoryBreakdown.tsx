...
export function RepositoryBreakdown({
  ...
  const reposByEcosystem = ecosystemScores.reduce(
    (acc, eco) => {
      if (eco.ecosystem !== "ALL") {
        acc[eco.ecosystem] = {
          ...
          avgScore:
            eco.repos.length > 0 ? eco.total_score / eco.repos.length : 0,
          ...
          <div className="space-y-4">
            {topRepos.map((repo, index) => {
              ...
              const repoDisplayName =
                repo.repo_name.split("/").pop() || repo.repo_name;
