...
function DeveloperRankGridView({ dataSource }: Pick<DeveloperRankViewWidgetProps, "dataSource">) {
  ...
  const getRankIcon = (index: number) => {
    switch (index) {
    case 0:
      return <Trophy size={20} className="text-yellow-600 dark:text-yellow-400" />;
    case 1:
      return <Medal size={20} className="text-gray-500 dark:text-gray-400" />;
    case 2:
      return <Award size={20} className="text-orange-600 dark:text-orange-400" />;
    default:
      return null;
    }
  };
      ...
      <Card className="bg-white dark:bg-surface-dark shadow-subtle border border-border dark:border-border-dark overflow-hidden">
        <CardHeader className="px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Top Developer Activity</h3>
          </div>
        </CardHeader>
            ...
            {displayedData.map((dev, index) => (
              ...
              >
                ...
                <div className="absolute -top-3 -right-3">
                  <div className={`w-10 h-10 rounded-full border-2 flex items-center justify-center font-bold text-sm shadow-sm ${getRankBadgeStyles(index)}`}>
