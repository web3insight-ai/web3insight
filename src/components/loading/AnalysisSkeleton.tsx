      ...
      {showUserInfo && (
        <div className="space-y-4">
          {Array.from({ length: userCount }).map((_, index) => (
            <div
              key={index}
              className="flex items-start space-x-4 p-4 border border-border dark:border-border-dark rounded-lg"
            >
              {/* Avatar skeleton */}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 3 }).map((_, index) => (
              <div
                key={index}
                className="p-4 border border-border dark:border-border-dark rounded-lg space-y-3"
              >
                {/* Ecosystem name */}
